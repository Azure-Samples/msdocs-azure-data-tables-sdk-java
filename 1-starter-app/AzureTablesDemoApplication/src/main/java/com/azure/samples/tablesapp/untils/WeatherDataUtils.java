...
package com.azure.samples.tablesapp.untils;
...
public class WeatherDataUtils {
    ...
    private static final List<String> EXCLUDE_TABLE_ENTITY_KEYS = new ArrayList<>() {{
        add("PartitionKey");
        add("RowKey");
        add("odata.etag");
        add("Timestamp");
    ...
    private static final List<String> DEFAULT_LIST_OF_KEYS = new ArrayList<>() {{
        add("Temperature");
        add("Humidity");
        add("Barometer");
        add("WindDirection");
    ...
    private static final List<String> MUST_FILTER_TABLE_ENTITY_KEYS = new ArrayList<>() {{
        add("odata.type");
        add("odata.id");
        add("odata.editLink");
    ...
    public static List<String> getListOfKeys(List<WeatherDataModel> weatherDataModelList) {
        ...
        weatherDataModelList.forEach(model -> model.getPropertyMap().keySet().forEach(key -> {
            if (listOfKeys.parallelStream().noneMatch(storiedKey -> storiedKey.equals(key))
                    && EXCLUDE_TABLE_ENTITY_KEYS.parallelStream().noneMatch(storiedKey -> storiedKey.equals(key))
                    && MUST_FILTER_TABLE_ENTITY_KEYS.parallelStream().noneMatch(storiedKey -> storiedKey.equals(key))) {
                listOfKeys.add(key);
            }
        }));
        return listOfKeys;
        ...
        weatherDataModelList.parallelStream().forEach(weatherDataModel ->
                listOfKeys.parallelStream().forEach(storedKey -> {
                    if (!weatherDataModel.getPropertyMap().containsKey(storedKey)) {
                        weatherDataModel.getPropertyMap().put(storedKey, BLANK_STRING);
                    }
                }));
    ...
    private static void rearrangeEntityProperties(Map<String, Object> target, Map<String, Object> source) {
        DEFAULT_LIST_OF_KEYS.forEach(key -> {
            if (source.containsKey(key)) {
                target.put(key, source.get(key));
            }
        });
        source.keySet().forEach(key -> {
            if (DEFAULT_LIST_OF_KEYS.parallelStream().noneMatch(defaultKey -> defaultKey.equals(key))
                    && EXCLUDE_TABLE_ENTITY_KEYS.parallelStream().noneMatch(defaultKey -> defaultKey.equals(key))) {
                target.put(key, source.get(key));
            }
        });
    }
